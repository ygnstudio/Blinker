...
- **工作区与桌面切换**：把当前窗口排布存成命名工作区，一键恢复原位（窗口管理页或悬浮按钮均可）；模拟 ⌃←/⌃→ 切换 macOS 桌面。
- **窗口管理**：悬停红绿灯后点「窗口管理」悬浮按钮，弹出九宫格摆放面板与工作区快速恢复，**始终作用于被悬停的窗口**；设置内可配置**拖拽贴靠**（拖到屏幕边缘/角落松手即贴靠）与**全局快捷键**（默认 ⌃⌥ + 方向键与 U/I/J/K，可录制改绑）。
- **悬停放大**：两种模式。**覆盖放大**绘制液态玻璃大按钮，带防误触进度环（0–800 ms 可调，环填满才响应，路过不误触）；**纯热区**保持标题栏原样，只扩大不可见点击区，点击立即响应。
- **玻璃托盘**：悬停放大时，一块无色液态玻璃胶囊托盘衬托整组放大按钮与扩展按钮，由系统合成器实时取景背后内容，任意标题栏背景都自然融合——无需采样、无需屏幕录制权限；放大灯珠为不透明纯色正圆，从源头杜绝偏色，并带同色受控辉光吸收原生按钮的玻璃残影。
...
├── Tests/BlinkerCoreTests/         # 单元测试（65 个）
├── docs/
...
3. 在**悬停放大** tab 选择模式与尺寸，玻璃托盘开箱即用、零额外权限。
...
Blinker 以 ad-hoc 签名分发（无付费开发者账号）。macOS 将辅助功能授权绑定到具体的代码签名，因此**每次升级到新版本后，系统设置里的开关仍显示"开启"，新版本却已被系统静默拒绝**——表现为点击拦截与悬停放大全部失效。修复方法：

```bash
tccutil reset Accessibility com.ygnstudio.Blinker
```

然后重新打开 Blinker，按提示重新授予辅助功能权限。Homebrew 升级与手动替换 App 均适用。
